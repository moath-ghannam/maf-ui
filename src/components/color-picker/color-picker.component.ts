...
import MfButton from '../button/button.component.js';
import MfButtonGroup from '../button-group/button-group.component.js';
import MfDropdown from '../dropdown/dropdown.component.js';
import MfIcon from '../icon/icon.component.js';
import MfInput from '../input/input.component.js';
import MfVisuallyHidden from '../visually-hidden/visually-hidden.component.js';
import ShoelaceElement from '../../internal/shoelace-element.js';
import styles from './color-picker.styles.js';
import type { CSSResultGroup } from 'lit';
import type { MfChangeEvent } from '../../events/mf-change.js';
import type { MfInputEvent } from '../../events/mf-input.js';
 ...
 * @since 2.0
 *
 * @dependency mf-button
 * @dependency mf-button-group
 * @dependency mf-dropdown
 * @dependency mf-input
 * @dependency mf-visually-hidden
 *
 * @slot label - The color picker's form label. Alternatively, you can use the `label` attribute.
 *
 * @event mf-blur - Emitted when the color picker loses focus.
 * @event mf-change - Emitted when the color picker's value changes.
 * @event mf-focus - Emitted when the color picker receives focus.
 * @event mf-input - Emitted when the color picker receives input.
 ...
 */
export default class MfColorPicker extends ShoelaceElement implements ShoelaceFormControl {
  ...
  @query('[part~="input"]') input: MfInput;
  @query('.color-dropdown') dropdown: MfDropdown;
  ...
  private handleInputChange(event: MfChangeEvent) {
    const target = event.target as HTMLInputElement;
    const oldValue = this.value;

    // Prevent the <mf-input>'s mf-change event from bubbling up
  ...
  private handleInputInput(event: MfInputEvent) {
    this.formControlController.updateValidity();

    // Prevent the <mf-input>'s mf-input event from bubbling up
    event.stopPropagation();
  }
  ...
  reportValidity() {
    if (!this.inline && !this.validity.valid) {
      ...
      this.dropdown.show();
      this.addEventListener('mf-after-show', () => this.input.reportValidity(), { once: true });

      if (!this.disabled) {
        // By standards we have to emit a `mf-invalid` event here synchronously.
  ...
  render() {
      ...
      >
        ...
        <div class="color-picker__user-input" aria-live="polite">
          <mf-input
            ...
            @mf-change=${this.handleInputChange}
            @mf-input=${this.handleInputInput}
            @mf-invalid=${this.handleInputInvalid}
            @mf-blur=${this.stopNestedEventPropagation}
            @mf-focus=${this.stopNestedEventPropagation}
          ></mf-input>
    ...
    return html`
      <mf-dropdown
        ...
        ?disabled=${this.disabled}
        ?hoist=${this.hoist}
        @mf-after-hide=${this.handleAfterHide}
      >
        <button
          part="trigger"
          slot="trigger"
