 ...
 * @since 2.0
 *
 * @event mf-load - Emitted when the included file is loaded.
 * @event {{ status: number }} mf-error - Emitted when the included file fails to load due to an error.
 */
export default class MfInclude extends ShoelaceElement {
  ...
  async handleSrcChange() {
    try {
      ...
      if (!file.ok) {
        this.emit('mf-error', { detail: { status: file.status } });
        return;
      }
      ...
      this.emit('mf-error', { detail: { status: -1 } });
    }
  }
