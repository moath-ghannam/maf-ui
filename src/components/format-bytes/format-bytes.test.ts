...
import type MfFormatBytes from './format-bytes.js';

describe('<mf-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<MfFormatBytes>(html` <mf-format-bytes></mf-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<MfFormatBytes>(html` <mf-format-bytes unit="bit"></mf-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
