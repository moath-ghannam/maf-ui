...
import type MfTree from './tree.component.js';
import type MfTreeItem from '../tree-item/tree-item.js';
...
describe('<mf-tree>', () => {
  ...
  beforeEach(async () => {
    el = await fixture(html`
      <mf-tree>
        <mf-tree-item>Node 1</mf-tree-item>
        <mf-tree-item>Node 2</mf-tree-item>
        <mf-tree-item id="expandable">
          Parent Node
          <mf-tree-item>Child Node 1</mf-tree-item>
          <mf-tree-item>
            Child Node 2
            <mf-tree-item>Child Node 2 - 1</mf-tree-item>
            <mf-tree-item>Child Node 2 - 2</mf-tree-item>
          </mf-tree-item>
        </mf-tree-item>
        <mf-tree-item>Node 3</mf-tree-item>
      </mf-tree>
    `);
  });
  ...
  it('should not focus collapsed nodes', async () => {
    // Arrange
    const parentNode = el.children[2] as MfTreeItem;
    const childNode = parentNode.children[1] as MfTreeItem;
  ...
  describe('when a custom expanded/collapsed icon is provided', () => {
    ...
    it('should append a clone of the icon in the proper slot of the tree item', async () => {
      ...
      // Act
      const treeItems = [...el.querySelectorAll('mf-tree-item')];
  ...
  describe('Keyboard navigation', () => {
    ...
    describe('when Home is pressed', () => {
      it('should move the focus to the first tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[3] as MfTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when End is pressed', () => {
      it('should move the focus to the last tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[0] as MfTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when Enter is pressed', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node = el.children[1] as MfTreeItem;
          node.focus();
          await el.updateComplete;
      ...
      describe('and selection is "leaf"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'leaf';
          const node = el.children[0] as MfTreeItem;
          node.focus();
          await el.updateComplete;
  ...
  describe('Interactions', () => {
    ...
    describe('when the user clicks the expand button', () => {
      it('should expand the tree item', async () => {
        ...
        const node = el.children[2] as MfTreeItem;
    ...
    describe('when the user clicks on a tree item', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node0 = el.children[0] as MfTreeItem;
          const node1 = el.children[1] as MfTreeItem;
      ...
      describe('and selection is "multiple"', () => {
        ...
        it('should set the indeterminate state to tree items if a child is selected', async () => {
          ...
          const parentNode = el.children[2] as MfTreeItem;
          const childNode = parentNode.children[0] as MfTreeItem;
    ...
    describe('when selection is "single"', () => {
      describe('and user clicks on same item twice', () => {
        it('should emit `mf-selection-change` event once', async () => {
          // Arrange
          el.selection = 'single';
          await el.updateComplete;

          const selectedChangeSpy = sinon.spy();
          el.addEventListener('mf-selection-change', selectedChangeSpy);
  ...
  describe('when selection is "leaf"', () => {
    ...
    describe('and user clicks on expandable item', () => {
      it('should not emit `mf-selection-change` event', async () => {
        // Arrange
        el.selection = 'leaf';
        await el.updateComplete;
        ...
        const node = el.querySelector<MfTreeItem>('#expandable')!;
  ...
  describe('when selection is "multiple"', () => {
    describe('and user clicks on same item twice', () => {
      it('should emit `mf-selection-change` event twice', async () => {
        // Arrange
        el.selection = 'multiple';
        await el.updateComplete;
    ...
    describe('when the tree gets initialized', () => {
      describe('and a parent node is selected', () => {
        it('should select all the nested children', async () => {
          // Arrange
          const tree = await fixture<MfTree>(html`
            <mf-tree selection="multiple">
              <mf-tree-item selected>
                Parent Node
                <mf-tree-item selected>Child Node 1</mf-tree-item>
                <mf-tree-item>
                  Child Node 2
                  <mf-tree-item>Child Node 2 - 1</mf-tree-item>
                  <mf-tree-item>Child Node 2 - 2</mf-tree-item>
          ...
          `);
          const treeItems = Array.from<MfTreeItem>(tree.querySelectorAll('mf-tree-item'));
