...
import MfTreeItem from '../tree-item/tree-item.component.js';
import ShoelaceElement from '../../internal/shoelace-element.js';
import styles from './tree.styles.js';
import type { CSSResultGroup } from 'lit';

function syncCheckboxes(changedTreeItem: MfTreeItem, initialSync = false) {
  function syncParentItem(treeItem: MfTreeItem) {
  ...
  function syncAncestors(treeItem: MfTreeItem) {
    const parentItem: MfTreeItem | null = treeItem.parentElement as MfTreeItem;

    if (MfTreeItem.isTreeItem(parentItem)) {
      syncParentItem(parentItem);
      syncAncestors(parentItem);
    }
  }

  function syncDescendants(treeItem: MfTreeItem) {
 ...
 * @since 2.0
 *
 * @event {{ selection: MfTreeItem[] }} mf-selection-change - Emitted when a tree item is selected or deselected.
 *
 * @slot - The default slot.
 * @slot expand-icon - The icon to show when the tree item is expanded. Works best with `<mf-icon>`.
 * @slot collapse-icon - The icon to show when the tree item is collapsed. Works best with `<mf-icon>`.
 *
 * @csspart base - The component's base wrapper.
 *
 * @cssproperty [--indent-size=var(--mf-spacing-medium)] - The size of the indentation for nested items.
 * @cssproperty [--indent-guide-color=var(--mf-color-neutral-200)] - The color of the indentation line.
 * @cssproperty [--indent-guide-offset=0] - The amount of vertical spacing to leave between the top and bottom of the
 *  indentation line's starting position.
 * @cssproperty [--indent-guide-style=solid] - The style of the indentation line, e.g. solid, dotted, dashed.
 * @cssproperty [--indent-guide-width=0] - The width of the indentation line.
 */
export default class MfTree extends ShoelaceElement {
  ...
  //
  private lastFocusedItem: MfTreeItem | null;
  private readonly localize = new LocalizeController(this);
  private mutationObserver: MutationObserver;
  private clickTarget: MfTreeItem | null = null;

  constructor() {
    super();
    this.addEventListener('focusin', this.handleFocusIn);
    this.addEventListener('focusout', this.handleFocusOut);
    this.addEventListener('mf-lazy-change', this.handleSlotChange);
  }
  ...
  private initTreeItem = (item: MfTreeItem) => {
  ...
  private handleTreeChanged = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      const addedNodes: MfTreeItem[] = [...mutation.addedNodes].filter(MfTreeItem.isTreeItem) as MfTreeItem[];
      const removedNodes = [...mutation.removedNodes].filter(MfTreeItem.isTreeItem) as MfTreeItem[];
  ...
  private selectItem(selectedItem: MfTreeItem) {
    ...
    if (
      ...
      Promise.all(nextSelection.map(el => el.updateComplete)).then(() => {
        this.emit('mf-selection-change', { detail: { selection: nextSelection } });
      });
    }
  }

  private getAllTreeItems() {
    return [...this.querySelectorAll<MfTreeItem>('mf-tree-item')];
  }

  private focusItem(item?: MfTreeItem | null) {
    item?.focus();
  }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (items.length > 0) {
      ...
      const activeItem: MfTreeItem | undefined = items[activeItemIndex];
  ...
  private handleClick(event: Event) {
    const target = event.target as MfTreeItem;
    const treeItem = target.closest('mf-tree-item')!;
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    this.clickTarget = event.target as MfTreeItem;
  }
  ...
  private handleFocusIn = (event: FocusEvent) => {
    ...
    if (MfTreeItem.isTreeItem(target) && !target.disabled) {
  ...
  async handleSelectionChange() {
    ...
    if (isSelectionMultiple) {
      ...
      [...this.querySelectorAll(':scope > mf-tree-item')].forEach((treeItem: MfTreeItem) =>
  ...
  get selectedItems(): MfTreeItem[] {
    const items = this.getAllTreeItems();
    const isSelected = (item: MfTreeItem) => item.selected;
  ...
  getFocusableItems() {
    ...
    return items.filter(item => {
      ...
      const parent: MfTreeItem | null | undefined = item.parentElement?.closest('[role=treeitem]');
