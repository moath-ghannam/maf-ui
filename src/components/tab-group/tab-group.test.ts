...
import type { MfTabShowEvent } from '../../events/mf-tab-show.js';
import type MfTab from '../tab/tab.js';
import type MfTabGroup from './tab-group.js';
import type MfTabPanel from '../tab-panel/tab-panel.js';
...
const waitForScrollButtonsToBeRendered = async (tabGroup: MfTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('mf-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: MfTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<MfTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('mf-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: MfTabPanel) => element.hasAttribute('active'));
...
const expectPromiseToHaveName = async (showEventPromise: Promise<MfTabShowEvent>, expectedName: string) => {
  const showEvent = await showEventPromise;
  expect(showEvent.detail.name).to.equal(expectedName);
};

const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<MfTab> => {
  const generalHeader = queryByTestId<MfTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<mf-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<MfTabGroup>(html`
      <mf-tab-group>
        <mf-tab slot="nav" panel="general" data-testid="general-tab-header">General</mf-tab>
        <mf-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</mf-tab>
        <mf-tab-panel name="general">This is the general tab panel.</mf-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<MfTabGroup>(html`
      <mf-tab-group>
        ...
        <mf-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</mf-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<mf-tab slot="nav" panel="tab-${i}">Tab ${i}</mf-tab>
            <mf-tab-panel name="tab-${i}">Content of tab ${i}0</mf-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`<mf-tab-group> ${generateTabs(30)} </mf-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`<mf-tab-group> ${generateTabs(2)} </mf-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`<mf-tab-group> ${generateTabs(50)} </mf-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<MfTabGroup>(
        html`<mf-tab-group> ${generateTabs(numberOfElements)} </mf-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('mf-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: MfTabGroup, action: () => Promise<void>): Promise<void> => {
      const generalHeader = await waitForHeaderToBeActive(tabGroup, 'general-header');
      generalHeader.focus();

      const customHeader = queryByTestId<MfTab>(tabGroup, 'custom-header');
      expect(customHeader).not.to.have.attribute('active');

      const showEventPromise = oneEvent(tabGroup, 'mf-tab-show') as Promise<MfTabShowEvent>;
      await action();
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'mf-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'mf-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`
        <mf-tab-group>
          <mf-tab slot="nav" panel="general" data-testid="general-header">General</mf-tab>
          <mf-tab slot="nav" panel="custom" data-testid="custom-header">Custom</mf-tab>
          <mf-tab-panel name="general">This is the general tab panel.</mf-tab-panel>
          <mf-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</mf-tab-panel>
        </mf-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`
        <mf-tab-group>
          <mf-tab slot="nav" panel="general" data-testid="general-header">General</mf-tab>
          <mf-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</mf-tab>
          <mf-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</mf-tab-panel>
          <mf-tab-panel name="disabled">This is the disabled tab panel.</mf-tab-panel>
        </mf-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<MfTabGroup>(html`
        <mf-tab-group>
          ...
          <mf-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</mf-tab-panel>
          <mf-tab-panel name="disabled">This is the custom tab panel.</mf-tab-panel>
        </mf-tab-group>
      `);
