...
import MfIcon from '../icon/icon.component.js';
import ShoelaceElement from '../../internal/shoelace-element.js';
import styles from './carousel.styles.js';
import type { CSSResultGroup, PropertyValueMap } from 'lit';
import type MfCarouselItem from '../carousel-item/carousel-item.component.js';
 ...
 * @dependency mf-icon
 *
 * @event {{ index: number, slide: MfCarouselItem }} mf-slide-change - Emitted when the active slide changes.
 *
 * @slot - The carousel's main content, one or more `<mf-carousel-item>` elements.
 * @slot next-icon - Optional next icon to use instead of the default. Works best with `<mf-icon>`.
 * @slot previous-icon - Optional previous icon to use instead of the default. Works best with `<mf-icon>`.
 *
 ...
 */
export default class MfCarousel extends ShoelaceElement {
  static styles: CSSResultGroup = [componentStyles, styles];
  static dependencies = { 'mf-icon': MfIcon };
  ...
  protected willUpdate(changedProperties: PropertyValueMap<MfCarousel> | Map<PropertyKey, unknown>): void {
  ...
  private getPageCount() {
    const slidesCount = this.getMfides().length;
  ...
  private getCurrentPage() {
    return Math.ceil(this.activeMfide / this.slidesPerMove);
  }
  ...
  private getMfides({ excludeClones = true }: { excludeClones?: boolean } = {}) {
  ...
  private handleKeyDown(event: KeyboardEvent) {
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      if (event.key === 'End') {
        this.goToMfide(this.getMfides().length - 1);
      }
  ...
  private handleScrollEnd() {
    ...
    if (this.loop && firstIntersecting?.target.hasAttribute('data-clone')) {
      ...
      this.goToMfide(clonePosition, 'instant');
    } else if (firstIntersecting) {
      const slides = this.getMfides();

      // Update the current index based on the first visible slide
      const slideIndex = slides.indexOf(firstIntersecting.target as MfCarouselItem);
      // Set the index to the first "snappable" slide
      this.activeMfide = Math.ceil(slideIndex / this.slidesPerMove) * this.slidesPerMove;
    }

    this.scrolling = false;
  }

  private isCarouselItem(node: Node): node is MfCarouselItem {
    return node instanceof Element && node.tagName.toLowerCase() === 'mf-carousel-item';
  }
  ...
  initializeMfides() {
    ...
    this.getMfides({ excludeClones: false }).forEach((slide, index) => {
    ...
    this.getMfides({ excludeClones: false }).forEach(slide => {
      intersectionObserver.observe(slide);
    });

    // Because the DOM may be changed, restore the scroll position to the active slide
    this.goToMfide(this.activeMfide, 'auto');
  }
  ...
  private createClones() {
    ...
    const lastMfides = slides.slice(-slidesPerPage);
    const firstMfides = slides.slice(0, slidesPerPage);

    lastMfides.reverse().forEach((slide, i) => {
      const clone = slide.cloneNode(true) as HTMLElement;
      clone.setAttribute('data-clone', String(slides.length - i - 1));
      this.prepend(clone);
    });

    firstMfides.forEach((slide, i) => {
      const clone = slide.cloneNode(true) as HTMLElement;
  ...
  handelMfideChange() {
    ...
    slides.forEach((slide, i) => {
      slide.classList.toggle('--is-active', i === this.activeMfide);
    });

    // Do not emit an event on first render
    if (this.hasUpdated) {
      this.emit('mf-slide-change', {
        detail: {
          index: this.activeMfide,
          slide: slides[this.activeMfide]
        }
      });
    }
  }
  ...
  previous(behavior: ScrollBehavior = 'smooth') {
    this.goToMfide(this.activeMfide - this.slidesPerMove, behavior);
  }
   ...
   * @param behavior - The behavior used for scrolling.
   */
  goToMfide(index: number, behavior: ScrollBehavior = 'smooth') {
    const { slidesPerPage, loop } = this;

    const slides = this.getMfides();
    const slidesWithClones = this.getMfides({ excludeClones: false });
    ...
    const newActiveMfide = loop ? (index + slides.length) % slides.length : clamp(index, 0, slides.length - 1);
    this.activeMfide = newActiveMfide;

    // Get the index of the next slide. For looping carousel it adds `slidesPerPage`
    // to normalize the starting index in order to ignore the first nth clones.
    const nextMfideIndex = clamp(index + (loop ? slidesPerPage : 0), 0, slidesWithClones.length - 1);
    const nextMfide = slidesWithClones[nextMfideIndex];

    this.scrollToMfide(nextMfide, prefersReducedMotion() ? 'auto' : behavior);
  }

  private scrollToMfide(slide: HTMLElement, behavior: ScrollBehavior = 'smooth') {
    const scrollContainer = this.scrollContainer;
    const scrollContainerRect = scrollContainer.getBoundingClientRect();
    const nextMfideRect = slide.getBoundingClientRect();

    const nextLeft = nextMfideRect.left - scrollContainerRect.left;
    const nextTop = nextMfideRect.top - scrollContainerRect.top;
          ...
          ? html`
              <div part="navigation" class="carousel__navigation">
                <button
                  ...
                  aria-label="${this.localize.term('previousMfide')}"
                  aria-controls="scroll-container"
                  aria-disabled="${prevEnabled ? 'false' : 'true'}"
                  @click=${prevEnabled ? () => this.previous() : null}
                >
                  <slot name="previous-icon">
                    <mf-icon library="system" name="${isLtr ? 'chevron-left' : 'chevron-right'}"></mf-icon>
                  </slot>
                </button>
                ...
                <button
                  ...
                  aria-label="${this.localize.term('nextMfide')}"
                  aria-controls="scroll-container"
                  aria-disabled="${nextEnabled ? 'false' : 'true'}"
                  @click=${nextEnabled ? () => this.next() : null}
                >
                  <slot name="next-icon">
                    <mf-icon library="system" name="${isLtr ? 'chevron-right' : 'chevron-left'}"></mf-icon>
                  </slot>
                </button>
              </div>
            `
                ...
                ${map(range(pagesCount), index => {
                  ...
                  return html`
                    <button
                      ...
                      aria-label="${this.localize.term('goToMfide', index + 1, pagesCount)}"
                      tabindex=${isActive ? '0' : '-1'}
                      @click=${() => this.goToMfide(index * slidesPerMove)}
                      @keydown=${this.handleKeyDown}
                    ></button>
                  `;
                })}
