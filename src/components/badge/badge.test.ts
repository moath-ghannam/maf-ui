...
import type MfBadge from './badge.js';
...
describe('<mf-badge>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<MfBadge>(html` <mf-badge>Badge</mf-badge> `);
    });
  ...
  describe('when provided a pill parameter', () => {
    before(async () => {
      el = await fixture<MfBadge>(html` <mf-badge pill>Badge</mf-badge> `);
    });
  ...
  describe('when provided a pulse parameter', () => {
    before(async () => {
      el = await fixture<MfBadge>(html` <mf-badge pulse>Badge</mf-badge> `);
    });
  ...
  ['primary', 'success', 'neutral', 'warning', 'danger'].forEach(variant => {
    describe(`when passed a variant attribute ${variant}`, () => {
      before(async () => {
        el = await fixture<MfBadge>(html`<mf-badge variant="${variant}">Badge</mf-badge>`);
      });
