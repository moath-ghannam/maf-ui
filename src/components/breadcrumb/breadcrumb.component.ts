...
import type MfBreadcrumbItem from '../breadcrumb-item/breadcrumb-item.js';
 ...
 * @slot separator - The separator to use between breadcrumb items. Works best with `<mf-icon>`.
 *
 * @dependency mf-icon
 *
 * @csspart base - The component's base wrapper.
 */
export default class MfBreadcrumb extends ShoelaceElement {
  ...
  private handleSlotChange() {
    const items = [...this.defaultSlot.assignedElements({ flatten: true })].filter(
      item => item.tagName.toLowerCase() === 'mf-breadcrumb-item'
    ) as MfBreadcrumbItem[];
  ...
  render() {
    ...
    return html`
      ...
      <span hidden aria-hidden="true">
        <slot name="separator">
          <mf-icon name=${this.localize.dir() === 'rtl' ? 'chevron-left' : 'chevron-right'} library="system"></mf-icon>
        </slot>
      </span>
    `;
  }
