...
import type MfSplitPanel from './split-panel.js';

const DIVIDER_WIDTH_IN_PX = 4;

const getPanel = (splitPanel: MfSplitPanel, testid: string): HTMLElement => {
  const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
  expect(startPanel).not.to.be.null;
  return startPanel!;
};

const getPanelWidth = (splitPanel: MfSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { width } = panel.getBoundingClientRect();
  return width;
};

const getPanelHeight = (splitPanel: MfSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { height } = panel.getBoundingClientRect();
  return height;
};

const getDivider = (splitPanel: MfSplitPanel): Element => {
  const divider = splitPanel.shadowRoot?.querySelector('[part="divider"]');
  expect(divider).not.to.be.null;
  return divider!;
};

describe('<mf-split-panel>', () => {
  afterEach(async () => {
    await resetMouse();
  });

  it('should render a component', async () => {
    const splitPanel = await fixture(html` <mf-split-panel></mf-split-panel> `);
  ...
  describe('panel sizing horizontal', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel>
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel position="25">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </mf-split-panel>`
      );
    ...
    it('emits the mf-reposition	event on position change', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel>
          <div slot="start">Start</div>
          <div slot="end">End</div>
      ...
      const repositionPromise = oneEvent(splitPanel, 'mf-reposition');
      splitPanel.position = 10;
  ...
  describe('panel sizing vertical', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel vertical style="height: 400px;">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </mf-split-panel>`
      );
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel position="25" vertical style="height: 400px;">
          <div slot="start" data-testid="start-panel">Start</div>
          <div slot="end" data-testid="end-panel">End</div>
        </mf-split-panel>`
      );
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<MfSplitPanel>(
        html`<mf-split-panel disabled vertical style="height: 400px;">
          <div slot="start">Start</div>
          <div slot="end">End</div>
        </mf-split-panel>`
      );
