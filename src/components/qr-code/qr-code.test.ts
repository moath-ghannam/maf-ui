...
import type MfQrCode from './qr-code.js';

const getCanvas = (qrCode: MfQrCode): HTMLCanvasElement => {
...
const expectCanvasToHaveAriaLabel = (qrCode: MfQrCode, expectedLabel: string): void => {
...
const getQrCodeColors = (qrCode: MfQrCode): QrCodeColors => {
  const canvas = getCanvas(qrCode);
...
const expectQrCodeColorsToBe = (qrCode: MfQrCode, expectedColors: QrCodeColors): void => {
...
describe('<mf-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<MfQrCode>(html` <mf-qr-code value="test data"></mf-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<MfQrCode>(html` <mf-qr-code value="test data" label="test label"></mf-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<MfQrCode>(html` <mf-qr-code value="test data" fill="red"></mf-qr-code>`);

    expectQrCodeColorsToBe(qrCode, { foreground: red, background: white });
  });

  it('sets the correct background for the qr code', async () => {
    const qrCode = await fixture<MfQrCode>(
      html` <mf-qr-code value="test data" fill="red" background="blue"></mf-qr-code>`
    );
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<MfQrCode>(html` <mf-qr-code value="test data" size="100"></mf-qr-code>`);
