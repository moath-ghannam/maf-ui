...
import type MfPopup from '../popup/popup.js';
...
export class SubmenuController implements ReactiveController {
  private host: ReactiveControllerHost & MfMenuItem;
  private popupRef: Ref<MfPopup> = createRef();
  ...
  constructor(
    host: ReactiveControllerHost & MfMenuItem,
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('mf-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('mf-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("mf-menu-item, [role^='menuitem']");
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (event.target === this.host) {
      ...
      (event.target.tagName === 'mf-menu-item' || event.target.role?.startsWith('menuitem'))
    ) {
      this.disableSubmenu();
    }
  };
  ...
  private handlePopupReposition = () => {
    const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
    const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'mf-menu')[0];
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <mf-popup>
    if (!this.isConnected) {
