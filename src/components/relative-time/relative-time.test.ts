...
import type MfRelativeTime from './relative-time.js';

interface MfRelativeTimeTestCase {
  date: Date;
  expectedOutput: string;
}

const extractTimeElement = (relativeTime: MfRelativeTime): HTMLTimeElement | null => {
  return relativeTime.shadowRoot?.querySelector('time') || null;
};

const expectFormattedRelativeTimeToBe = async (relativeTime: MfRelativeTime, expectedOutput: string): Promise<void> => {
...
const createRelativeTimeWithDate = async (relativeDate: Date): Promise<MfRelativeTime> => {
  const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
    <mf-relative-time lang="en-US"></mf-relative-time>
  `);
  relativeTime.date = relativeDate;
  return relativeTime;
};

const expectTitleToMatchLocalizedTimeString = (relativeTime: MfRelativeTime) => {
...
const testCases: MfRelativeTimeTestCase[] = [
  {
...
describe('mf-relative-time', () => {
  ...
  describe('handles time correctly', () => {
    ...
    testCases.forEach(testCase => {
      ...
      it(`shows the correct relative time given a String object: ${testCase.expectedOutput}`, async () => {
        ...
        const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
          <mf-relative-time lang="en-US" date="${dateString}"></mf-relative-time>
        `);
    ...
    it('always shows numeric if requested via numeric property', async () => {
      const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
        <mf-relative-time lang="en-US" numeric="always"></mf-relative-time>
      `);
    ...
    it('shows human readable form if appropriate and numeric property is auto', async () => {
      const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
        <mf-relative-time lang="en-US" numeric="auto"></mf-relative-time>
      `);
      relativeTime.date = yesterday;
    ...
    it('allows to use a short form of the unit', async () => {
      ...
      const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
        <mf-relative-time lang="en-US" numeric="always" format="short"></mf-relative-time>
      `);
    ...
    it('allows to use a long form of the unit', async () => {
      const twoYearsAgo = new Date(currentTime.getTime() - 2 * nonLeapYearInSeconds);
      const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
        <mf-relative-time lang="en-US" numeric="always" format="long"></mf-relative-time>
      `);
      relativeTime.date = twoYearsAgo;
    ...
    it('is formatted according to the requested locale', async () => {
      const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
        <mf-relative-time lang="de-DE" numeric="auto"></mf-relative-time>
      `);
      relativeTime.date = yesterday;
  ...
  it('does not display a time element on invalid time string', async () => {
    ...
    const relativeTime: MfRelativeTime = await fixture<MfRelativeTime>(html`
      <mf-relative-time lang="en-US" date="${invalidDateString}"></mf-relative-time>
    `);
