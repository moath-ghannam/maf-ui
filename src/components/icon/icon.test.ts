...
import type { MfErrorEvent } from '../../events/mf-error.js';
import type { MfLoadEvent } from '../../events/mf-load.js';
import type MfIcon from './icon.js';
...
describe('<mf-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon></mf-icon> `);
    ...
    it('renders pre-loaded system icons and emits mf-load event', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="system"></mf-icon> `);
      const listener = oneEvent(el, 'mf-load') as Promise<MfLoadEvent>;
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="system" name="check"></mf-icon> `);
  ...
  describe('when a label is provided', () => {
    it('the icon has the correct default aria attributes', async () => {
      ...
      const el = await fixture<MfIcon>(html` <mf-icon label="${fakeLabel}" library="system" name="check"></mf-icon> `);
  ...
  describe('when a valid src is provided', () => {
    it('the svg is rendered', async () => {
      ...
      const listener = oneEvent(el, 'mf-load');
  ...
  describe('new library', () => {
    it('renders icons from the new library and emits mf-load event', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="test-library"></mf-icon> `);
      const listener = oneEvent(el, 'mf-load') as Promise<MfLoadEvent>;
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="test-library" name="test-icon1"></mf-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="test-library" name="does-not-exist"></mf-icon> `);

      expect(el.shadowRoot?.querySelector('svg')).to.be.null;
    });

    it('emits mf-error when the file cant be retrieved', async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="test-library"></mf-icon> `);
      const listener = oneEvent(el, 'mf-error') as Promise<MfErrorEvent>;
    ...
    it("emits mf-error when there isn't an svg element in the registered icon", async () => {
      const el = await fixture<MfIcon>(html` <mf-icon library="test-library"></mf-icon> `);
      const listener = oneEvent(el, 'mf-error') as Promise<MfErrorEvent>;
  ...
  describe('svg sprite sheets', () => {
    ...
    it('Should properly grab an SVG and render it from bootstrap icons', async () => {
      ...
      const el = await fixture<MfIcon>(html`<mf-icon name="arrow-left" library="sprite"></mf-icon>`);
    ...
    it('Should render nothing if the sprite hash is wrong', async () => {
      ...
      const el = await fixture<MfIcon>(html`<mf-icon name="non-existent" library="sprite"></mf-icon>`);
    ...
    it.skip("Should produce an error if the icon doesn't exist.", async () => {
      ...
      const el = await fixture<MfIcon>(html`<mf-icon name="bad-icon" library="sprite"></mf-icon>`);
      const listener = oneEvent(el, 'mf-error') as Promise<MfErrorEvent>;
