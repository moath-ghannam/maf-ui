...
import type MfImageComparer from './image-comparer.js';

describe('<mf-image-comparer>', () => {
  it('should render a basic before/after', async () => {
    const el = await fixture<MfImageComparer>(html`
      <mf-image-comparer>
        <div slot="before"></div>
        <div slot="after"></div>
      </mf-image-comparer>
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('mf-change', handler, { once: true });
