...
import type MfTreeItem from './tree-item.js';

describe('<mf-tree-item>', () => {
  let leafItem: MfTreeItem;
  let parentItem: MfTreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <mf-tree-item>Node 1</mf-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit mf-expand and mf-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('mf-expand', expandSpy);
        parentItem.addEventListener('mf-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit mf-collapse and mf-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('mf-collapse', collapseSpy);
        parentItem.addEventListener('mf-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'mf-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit mf-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('mf-lazy-change', lazyChangeSpy);
