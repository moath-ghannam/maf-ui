...
import type MfAvatar from './avatar.js';
...
describe('<mf-avatar>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<MfAvatar>(html` <mf-avatar label="Avatar"></mf-avatar> `);
    });
  ...
  describe('when provided an image and label parameter', () => {
    ...
    before(async () => {
      el = await fixture<MfAvatar>(html`<mf-avatar image="${image}" label="${label}"></mf-avatar>`);
    });

    it('should pass accessibility tests', async () => {
      /**
       * The image element itself is ancillary, because it's parent container contains the
       * aria-label which dictates what "mf-avatar" is. This also implies that label text will
  ...
  describe('when provided initials parameter', () => {
    const initials = 'SL';
    before(async () => {
      el = await fixture<MfAvatar>(html`<mf-avatar initials="${initials}" label="Avatar"></mf-avatar>`);
    });
  ...
  describe('when image is present, the initials or icon part should not render', () => {
    ...
    const label = 'Small transparent square';
    before(async () => {
      el = await fixture<MfAvatar>(
        html`<mf-avatar image="${image}" label="${label}" initials="${initials}"></mf-avatar>`
      );
    });
  ...
  ['square', 'rounded', 'circle'].forEach(shape => {
    describe(`when passed a shape attribute ${shape}`, () => {
      before(async () => {
        el = await fixture<MfAvatar>(html`<mf-avatar shape="${shape}" label="Shaped avatar"></mf-avatar>`);
      });
  ...
  describe('when passed a <span>, on slot "icon"', () => {
    before(async () => {
      el = await fixture<MfAvatar>(html`<mf-avatar label="Avatar"><span slot="icon">random content</span></mf-avatar>`);
    });
  ...
  it('should not render the image when the image fails to load', async () => {
    el = await fixture<MfAvatar>(html`<mf-avatar></mf-avatar>`);
    el.image = 'bad_image';
