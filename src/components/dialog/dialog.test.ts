...
import type MfDialog from './dialog.js';

describe('<mf-dialog>', () => {
  it('should be visible with the open attribute', async () => {
    const el = await fixture<MfDialog>(html`
      <mf-dialog open>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</mf-dialog>
    `);
  ...
  it('should not be visible without the open attribute', async () => {
    const el = await fixture<MfDialog>(html`
      <mf-dialog>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</mf-dialog>
    `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should allow initial focus to be set', async () => {
    const el = await fixture<MfDialog>(html` <mf-dialog><input /></mf-dialog> `);
  ...
  it('should close when pressing Escape', async () => {
    const el = await fixture<MfDialog>(html` <mf-dialog open></mf-dialog> `);
    const hideHandler = sinon.spy();
  ...
  it('should properly cycle through tabbable elements when mf-dialog is used in a shadowRoot', async () => {
    class AContainer extends LitElement {
      get dialog() {
        return this.shadowRoot?.querySelector('mf-dialog');
      }
      ...
      render() {
        return html`
          ...
          <mf-button @click=${this.openDialog}>Open Dialog</mf-button>
        `;
      }
    }
    ...
    const dialog = container.shadowRoot?.querySelector('mf-dialog');

    if (!dialog) {
      throw Error('Could not find <mf-dialog> element.');
    }
    ...
    // Opens modal.
    const openModalButton = container.shadowRoot?.querySelector('mf-button');
