...
import type MfTextarea from './textarea.js';

describe('<mf-textarea>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<MfTextarea>(html` <mf-textarea label="Name"></mf-textarea> `);
    await expect(el).to.be.accessible();
  });

  it('default properties', async () => {
    const el = await fixture<MfTextarea>(html` <mf-textarea></mf-textarea> `);
  ...
  it('should have title if title attribute is set', async () => {
    const el = await fixture<MfTextarea>(html` <mf-textarea title="Test"></mf-textarea> `);
  ...
  it('should be disabled with the disabled attribute', async () => {
    const el = await fixture<MfTextarea>(html` <mf-textarea disabled></mf-textarea> `);
  ...
  describe('when the value changes', () => {
    it('should emit mf-change and mf-input when the user types in the textarea', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea></mf-textarea> `);
    ...
    it('should not emit mf-change or mf-input when calling setRangeText()', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea value="hi there"></mf-textarea> `);
  ...
  describe('when using constraint validation', () => {
    ...
    it('should be invalid when required and empty', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea required></mf-textarea> `);

      expect(el.checkValidity()).to.be.false;
    });

    it('should be invalid when required and after removing disabled ', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea disabled required></mf-textarea> `);
    ...
    it('should receive the correct validation attributes ("states") when valid', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea required value="a"></mf-textarea> `);
    ...
    it('should receive validation attributes ("states") even when novalidate is used on the parent form', async () => {
      const el = await fixture<HTMLFormElement>(html` <form novalidate><mf-textarea required></mf-textarea></form> `);
      const textarea = el.querySelector<MfTextarea>('mf-textarea')!;
  ...
  describe('when submitting a form', () => {
    it('should serialize its name and value with FormData', async () => {
      const form = await fixture<HTMLFormElement>(html` <form><mf-textarea name="a" value="1"></mf-textarea></form> `);
    ...
    it('should be invalid when setCustomValidity() is called with a non-empty value', async () => {
      const textarea = await fixture<HTMLFormElement>(html` <mf-textarea></mf-textarea> `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <mf-textarea form="f" name="a" value="1"></mf-textarea>
        </div>
      `);
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <mf-textarea name="a" value="test"></mf-textarea>
          <mf-button type="reset">Reset</mf-button>
        </form>
      `);
      const button = form.querySelector('mf-button')!;
      const textarea = form.querySelector('mf-textarea')!;
      textarea.value = '1234';
  ...
  describe('when using spellcheck', () => {
    ...
    it('should enable spellcheck when set to "true"', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea spellcheck="true"></mf-textarea> `);
      const textarea = el.shadowRoot!.querySelector<HTMLTextAreaElement>('textarea')!;
      expect(textarea.getAttribute('spellcheck')).to.equal('true');
      expect(textarea.spellcheck).to.be.true;
    });
    ...
    it('should disable spellcheck when set to "false"', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea spellcheck="false"></mf-textarea> `);
      const textarea = el.shadowRoot!.querySelector<HTMLTextAreaElement>('textarea')!;
  ...
  describe('when using the setRangeText() function', () => {
    it('should set replacement text in the correct location', async () => {
      const el = await fixture<MfTextarea>(html` <mf-textarea value="test"></mf-textarea> `);
  ...
  runFormControlBaseTests('mf-textarea');
});
