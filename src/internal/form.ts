...
import type MfButton from '../components/button/button.js';
...
export class FormControlController implements ReactiveController {
  ...
  constructor(host: ReactiveControllerHost & ShoelaceFormControl, options?: Partial<FormControlControllerOptions>) {
    ...
    this.options = {
      form: input => {
        ...
        // Controls may not always reflect the 'form' property. For example, `<mf-button>` doesn't reflect.
  ...
  private handleFormData = (event: FormDataEvent) => {
    ...
    const isButton = this.host.tagName.toLowerCase() === 'mf-button';
  ...
  private doAction(type: 'submit' | 'reset', submitter?: HTMLInputElement | MfButton) {
    if (this.form) {
  ...
  reset(submitter?: HTMLInputElement | MfButton) {
    this.doAction('reset', submitter);
  }

  /** Submits the form, triggering validation and form data injection. */
  submit(submitter?: HTMLInputElement | MfButton) {
  ...
  /**
   * Dispatches a non-bubbling, cancelable custom event of type `mf-invalid`.
   * If the `mf-invalid` event will be cancelled then the original `invalid`
   * event (which may have been passed as argument) will also be cancelled.
   * If no original `invalid` event has been passed then the `mf-invalid`
   * event will be cancelled before being dispatched.
   */
  emitInvalidEvent(originalInvalidEvent?: Event) {
    const slInvalidEvent = new CustomEvent<Record<PropertyKey, never>>('mf-invalid', {
      bubbles: false,
      composed: false,
      cancelable: true,
      detail: {}
