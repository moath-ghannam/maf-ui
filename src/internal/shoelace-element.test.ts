...
import MfButton from '../../dist/components/button/button.component.js';

// We don't use ShoelaceElement directly because it shouldn't exist in the final bundle.
/* eslint-disable */
const ShoelaceElement = Object.getPrototypeOf(MfButton);
/* eslint-enable */
...
it('Should provide a console warning if attempting to register the same tag twice', () => {
  class MyButton extends MfButton {
    static version = '0.4.5';
  }

  const stub = Sinon.stub(console, 'warn');

  expect(Boolean(window.customElements.get('mf-button'))).to.be.false;
  /* eslint-disable */
  MfButton.define('mf-button');
  expect(Boolean(window.customElements.get('mf-button'))).to.be.true;
  MyButton.define('mf-button');
  /* eslint-enable */
  ...
  expect(warning).to.match(
    new RegExp(
      /* eslint-disable */
      `Attempted to register <mf-button> v${MyButton.version}, but <mf-button> v${MfButton.version} has already been registered`
      /* eslint-enable */
    ),
    'i'
  );
...
it('Should register dependencies when the element is constructed the first time', () => {
  ...
  class MyElement extends ShoelaceElement {
    static dependencies = { 'mf-button': MfButton };
  ...
  new (window.customElements.get('mf-element'))();
  /* eslint-enable */
