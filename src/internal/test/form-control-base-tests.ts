...
function runAllValidityTests(
  ...
  describe(`Form validity base test for ${displayName}`, async () => {
    ...
    it('should not emit an `mf-invalid` event when `.checkValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'mf-invalid', () => control.checkValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    it('should not emit an `mf-invalid` event when `.reportValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'mf-invalid', () => control.reportValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    // TODO: As soon as `MfRadioGroup` has a property `disabled` this
    // condition can be removed
    if (tagName !== 'mf-radio-group') {
      it('should not emit an `mf-invalid` event when `.checkValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
      ...
      it('should not emit an `mf-invalid` event when `.reportValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        control.disabled = true;
        await control.updateComplete;
...
//
//  Special tests for <mf-button type="button">
//
function runSpecialTests_slButtonOfTypeButton(createControl: CreateControlFn) {
  ...
  it('should not emit an `mf-invalid` event when `.checkValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
  ...
  it('should not emit an `mf-invalid` event when `.reportValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
//
// Special tests for <mf-button href="...">
//
function runSpecialTests_slButtonWithHref(createControl: CreateControlFn) {
  ...
  it('should not emit an `mf-invalid` event when `.checkValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'mf-invalid', () => control.checkValidity());
  ...
  it('should not emit an `mf-invalid` event when `.reportValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'mf-invalid', () => control.reportValidity());
...
function runSpecialTests_standard(createControl: CreateControlFn) {
  ...
  it('should emit an `mf-invalid` event when `.checkValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
  ...
  it('should emit an `mf-invalid` event when `.reportValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
// Component `mf-button` behaves quite different to the other components. To keep things simple we use simple conditions
// here. `mf-button` might stay the only component in Shoelace core behaves that way, so we just hard code it here.
function getMode(control: ShoelaceFormControl) {
  if (
    control.localName === 'mf-button' && //
    'href' in control &&
    'type' in control &&
    control.type === 'button' &&
    !control.href
  ...
  // <mf-button href="...">
  if (control.localName === 'mf-button' && 'href' in control && !!control.href) {
