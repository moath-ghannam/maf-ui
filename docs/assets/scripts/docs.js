...
(() => {
  ...
  function setTheme(newTheme) {
    ...
    document.documentElement.classList.toggle('mf-theme-dark', isDark());
  }

  function updateSelection() {
    const menu = document.querySelector('#theme-selector mf-menu');
    if (!menu) return;
    [...menu.querySelectorAll('mf-menu-item')].map(item => (item.checked = item.getAttribute('value') === theme));
  }

  let theme = getTheme();

  // Selection is not preserved when changing page, so update when opening dropdown
  document.addEventListener('mf-show', event => {
  ...
  document.addEventListener('mf-select', event => {
    const menu = event.target.closest('#theme-selector mf-menu');
    if (!menu) return;
