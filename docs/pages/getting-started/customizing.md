...
meta:
  title: Customizing
  description: Learn how to customize maf-ui through parts and custom properties.
---

# Customizing

maf-ui components can be customized at a high level through design tokens. This gives you control over theme colors and general styling. For more advanced customizations, you can make use of CSS parts and custom properties to target individual components.

## Design Tokens

maf-ui makes use of several design tokens to provide a consistent appearance across components. You can customize them and use them in your own application with pure CSS — no preprocessor required.
...
Design tokens are accessed through CSS custom properties that are defined in your theme. Because design tokens live at the page level, they're prefixed with `--mf-` to avoid collisions with other libraries.
...
:root {
  ...
  --mf-color-primary-50: var(--mf-color-purple-50);
  --mf-color-primary-100: var(--mf-color-purple-100);
  --mf-color-primary-200: var(--mf-color-purple-200);
  --mf-color-primary-300: var(--mf-color-purple-300);
  --mf-color-primary-400: var(--mf-color-purple-400);
  --mf-color-primary-500: var(--mf-color-purple-500);
  --mf-color-primary-600: var(--mf-color-purple-600);
  --mf-color-primary-700: var(--mf-color-purple-700);
  --mf-color-primary-800: var(--mf-color-purple-800);
  --mf-color-primary-900: var(--mf-color-purple-900);
  --mf-color-primary-950: var(--mf-color-purple-950);
}
```

Many design tokens are described further along in this documentation. For a complete list, refer to `src/themes/light.css` in the project's [source code](https://github.com/maf-ui/blob/current/src/themes/light.css).
...
maf-ui components use a [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM) to encapsulate their styles and behaviors. As a result, you can't simply target their internals with the usual CSS selectors. Instead, components expose "parts" that can be targeted with the [CSS part selector](https://developer.mozilla.org/en-US/docs/Web/CSS/::part), or `::part()`.

Here's an example that modifies buttons with the `tomato-button` class.

```html:preview
<mf-button class="tomato-button"> Tomato Button </mf-button>

<style>
  .tomato-button::part(base) {
    background: var(--mf-color-neutral-0);
...
For convenience, some components expose CSS custom properties you can override. These are not design tokens, nor do they have the same `--mf-` prefix since they're scoped to a component.
...
```html
<mf-avatar style="--size: 6rem;"></mf-avatar>
```
...
```js
import { setDefaultAnimation } from 'maf-ui-kit/dist/utilities/animation-registry.js';
...
```js
import { setAnimation } from 'maf-ui-kit/dist/utilities/animation-registry.js';
